 ...
 *
 * ## 這裡的元件都不含 <section> 與 .container
 *
 * 外層的留白與底色由 Home.jsx 決定，這樣所有區段的間距規則
 * 集中在同一個檔案裡看得到，不必逐一打開每個元件比對。
 */
...
const FEATURES = [
  ...
  { num: '04', title: '團購可客製', desc: '社團、公司行號大量訂購可分裝寄送、客製標籤，並開立農民收據。' },
]

const SectionHead = ({ eyebrow, title, desc, style, eyebrowStyle }) => (
  <div className="section-head" style={style}>
    {eyebrow && <div className="section-head__eyebrow" style={eyebrowStyle}>{eyebrow}</div>}
  ...
  return (
    <div className="story-row" style={{ marginBottom: 0 }}
      ...
      <div className="story-row__media">
        <Placeholder src={story.cover_url} fit="auto" ratio="4x3" alt={story.title}
                     hint={'故事照片\nstory-1.jpg'} />
      </div>
      <div>
        ...
        <Link to="/story" className="btn btn--outline" style={{ marginTop: 12 }}>閱讀完整故事</Link>
      </div>
    </div>
  )
}
...
export function HomeNews() {
  ...
  return (
    <>
      <SectionHead eyebrow="News & Media" title="最新消息與報導" />
      {news.length ? (
  ...
  return (
    <div className="text-center">
      <SectionHead eyebrow="Contact"
                   title="大量訂購或有任何問題"
                   desc="歡迎透過 LINE 或電話直接聯絡我們，我們會盡快回覆"
                   style={{ marginBottom: 24 }}
                   eyebrowStyle={{ color: 'var(--honey-300)' }} />
