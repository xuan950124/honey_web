...
| `/admin/stats` | **流量統計：每天幾個人來、看了什麼、從哪裡來** |
...
宅配（黑貓、中華郵政）的**收件人郵遞區號是綠界的必填欄位**，
少了就回 `ReceiverZipCode Is Null`，而且不會告訴你是哪一筆、少了哪一欄。

但客人不會填。結帳頁那欄是選填，十個有九個跳過 ——
他們心裡想的是「我地址都寫完整了你還要我查郵遞區號？」，這很合理。

所以系統自己查，全台 368 個鄉鎮市區都在表裡：

- **客人打地址的當下**就把郵遞區號帶進欄位，看得到也可以自己改
- **下單時**後端再算一次存進訂單
- **按建立物流單時**如果還是空的，從地址再查一次並補回訂單

三個時間點都查，是因為舊訂單（在這個功能之前成立的）沒有郵遞區號，
第三次補查讓它們不必手動改就能出貨。

> **同名的區會看縣市。** 中正區有台北（100）跟基隆（202）、
> 東區有四個（新竹 300、台中 401、嘉義 600、台南 701）。
...
**後台 → 流量統計**（`/admin/stats`）。不用裝 Google Analytics、不用放追蹤碼、
不用處理 cookie 同意視窗 —— 資料就存在你自己的資料庫裡。

看得到三件事：

- **每天的瀏覽次數與人數** —— 最近 7／30／90 天
- **最多人看的頁面** —— 哪支蜂蜜的商品頁被點最多
- **客人從哪裡來** —— Google、Facebook，還是直接輸入網址

### 你自己不會被算進去

這是這個功能最重要的一件事。你每天開自己的網站看排版看十次，
那些數字會蓋掉真實客人的樣子，統計就沒意義了。所以有四層排除：

| 排除的對象 | 怎麼做到的 |
|---|---|
| **登入中的工作人員** | 自動，不用設定。你在後台操作、逛前台都不算 |
| **這台裝置** | 統計頁最下面有個勾選框。登出或用無痕視窗看自己網站時會用到 |
| **爬蟲與監測服務** | Google、Facebook 的抓取程式，還有 curl、python-requests 這類 |
| **後台／購物車／訂單頁** | 那不是「客人在逛」，算進去只會讓數字虛胖 |

> 「這台裝置」的設定存在瀏覽器裡。換一台電腦、換一個瀏覽器、
> 或是清掉瀏覽資料之後，要重新勾一次。

### 為什麼人數會比你想的多

系統**不存 IP**。但要算「幾個人」又得分辨是不是同一個人，
所以存的是雜湊值，而且**每天換一組鹽**。

這代表：反推不回原始 IP，也追蹤不了跨日的同一個人 ——
代價是**同一個人今天來、明天再來，會被算成兩位訪客**。

所以「人數」看趨勢就好，別當成精確的人頭數。
瀏覽次數則是準的。

> 為什麼要這麼麻煩？IP 在個資法下是個人資料，存了就要面對
> 蒐集目的告知、保存期限、當事人查閱刪除權那一整套。
> 存不下來最省事。
...
| `page_views` | 瀏覽紀錄（不存 IP，只存每天換鹽的雜湊；保留 180 天） |
